...
async def get_history(symbol: str, period: str = "3mo"):
    ...
    try:
        ...
        return MarketData.get_historical_data(clean_symbol, yf_period, yf_interval)
    except ValueError as e:
...
async def get_news(symbol: str, days: int = 7):
    ...
    try:
        clean_symbol = validate_symbol(symbol)
        return await NewsFetcher.get_stock_news(clean_symbol, days)
    except ValueError as e:
...
async def get_financials(symbol: str):
    ...
    try:
        clean_symbol = validate_symbol(symbol)
        return Financials.get_key_metrics(clean_symbol)
    except ValueError as e:
        return {"error": str(e), "symbol": symbol}
...
async def get_company(symbol: str):
    ...
    try:
        clean_symbol = validate_symbol(symbol)
        return Financials.get_company_info(clean_symbol)
    except ValueError as e:
        return {"error": str(e), "symbol": symbol}
...
async def get_bulk_price(symbols: str):
    ...
    for symbol in symbol_list:
        try:
            ...
            data = MarketData.get_current_price(clean)
            results.append(data)
        except Exception:
